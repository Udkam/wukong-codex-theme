...
首帧持久绘制由两条路径共同保证：稳定原生选择器同步覆盖 composer、submit、选中侧栏与环境卡的基础 paint；当 React 保留这些根节点但替换内部子树，或新增 landing/thread surface evidence 时，MutationObserver 只合并一个微任务刷新，并在首个 rAF 前完成动态标记。纯文本流式节点不进入该快速路径，普通差量对账仍保持事件驱动，不引入 interval、轮询或整页重建。

`backgrounds.cmd` 是公开素材管理入口，提供交互及 `list/add/replace/move/remove`。管理器用稳定 slot 定位资产，用 order 重排播放位；add 自动分配未使用槽位，remove 只退出轮播并保留文件。它调用 `scripts/prepare-background.ps1` 做有界确定性转码，以 UTF-8 无 BOM 原子替换 `themes/active.json`，并把清单与覆盖前资产备份到 `.wukong-runtime/background-backups/`。管理动作不启动常驻进程、端口、WMI/CIM 或页面控件。

## V51.9 二十二图画廊、低遮罩与原色保留（历史）

V51.9 在不改变 V51.8 原位 `<img>` 解码路径、持久状态版本或事件源的前提下，把活动画廊扩展为战斗 `B01..B13` 与风景 `S01..S09`。B06/S04 在原索引处替换，B10–B13 追加，因此所有未替换场景的数字索引保持稳定；当时每组仍按编号顺序循环。
...
V51.8 保留 V49–V51.7 的低占用核心：普通任务/项目/history/hash/流式变化只触发表面对账，不选择新图。变化只来自两个显式事件源：真实“新建任务”点击在 20 分钟自动冷却通过后推进一组，`Ctrl+Alt+F` / `Ctrl+Alt+B` 手动前进或后退当前 battle 或 scenery 模式。没有后台计时轮播；冷却只在事件到来时比较时间戳，因此页面空闲时没有背景工作。
...
> **V52.1 现行候选设计。** V51.9 及更早章节保留为演变记录；冲突处以 `CURRENT_GOAL.md` 和顶部 V52.1 / V51.5 / V50 章节为准。公开联系邮箱已由当前 GitHub 账号确认；candidate 仅保留用户视觉验收发布标记。
...
V15 当时只组装 9 张背景：`erlang-ink-duel` 与 `great-sage-staff` 为主战斗池，`storm-bearer` 与 `shadow-confrontation` 为次级战斗池，5 张纯风景保持 thread 池。现行 V52.1 已扩展为 B01–B13 与 S01–S09 两组稳定槽位，并由独立 `order` 定义播放序列。默认/原生预览在 V15 当时统一指向 1920×1080 的 `great-sage-staff.jpg`；现行默认资源镜像 battle `order=1` 的 B07。用户否决的 `destined-afterimage.jpg`、`yaksha-king-rift.jpg` 与旧 1256×707 `great-sage-return.jpg` 均保留在本地素材/历史证据中，但不生成 CSS 变量、不嵌入 payload、不复制到最小运行包。

V15 当时收敛后的图库解码总量为 19,258,880 px，交叉淡变最坏两图为 4,743,680 px。现行 V52.1 的 22 图实测值分别为 45,201,592 px 与 5,337,600 px，仍只按需解码当前一图，过渡时最多保留两图；场景数量不会改变原生页面结构、路由判定或全窗 `cover` 合同。
...
现行资源合同不做相邻场景或跨模式预取。静态门限为单背景 12,000,000 px、图库唯一文件总计 48,000,000 px、最大两张合计 16,000,000 px、单 UI/装饰图 4,194,304 px、压缩图库总计 24 MiB。V52.1 的 22 图实测总计 45,201,592 px，最大两张合计 5,337,600 px，压缩文件合计 8,355,513 bytes；历史低分辨率或用户否决的候选图仍可原位保留，但不进入活动清单或最小包，不能把“文件仍在仓库”混写成“当前发布仍使用”。
