...
“下载即用、删除即原生”的可实现定义是：解压后双击包内 `start.cmd` 启动主题 Codex；双击 `stop-theme.cmd` 后关闭该窗口，用户可自行删除整个解压目录。所有 profile、请求、事件和运行状态均写在包内 `.wukong-runtime`，普通 Codex 入口不加载 CSS。0.8.0 不写官方程序、`config.toml` 或目录外快捷方式。按用户最高约束，本轮实现、测试和发布不得删除任何文件，旧版本与证据采用 append-only 保留。
