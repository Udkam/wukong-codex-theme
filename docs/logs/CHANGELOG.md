...
## 2026-09-22 — 原生玻璃材质与手动入口整理

- 统一顶栏与侧栏材质并沿用原生圆弧；修复设置搜索栏黑底及深浅色阅读遮罩。
- 修复设置往返触发全量重注入，合并区域刷新并保留已解码背景。
- 风景保留三张，默认雪山；唯一入口改为 start.cmd，原生启动与自动更新策略不变。
- 只同步主题源码；宠物改动和本地历史存档不纳入本次提交，不创建发行包或标签。

## 2026-08-16 — V75 限定明纸浮层纹理范围

- 修复 V74 不透明回退暴露的整页长纸：原生“已编辑文件”卡为 `position: static`，其绝对定位纸纹伪元素曾错误以整段对话列为包含块；现在仅为原生静态纸面叶节点建立局部包含块，纸纹不再越出卡片。
- 保留原生已定位菜单、弹窗及卡片的定位方式；静态节点改为 `position: relative` 不改变常规流尺寸和坐标，并由高外壳中的静态审阅卡回归用例锁定。
- 本机热更新为 `v75-bounded-paper-overlays`，不透明明纸默认、背景锁定、设置页遮罩及其他既有视觉保持不变。

## 2026-08-16 — V74 回退透明框体设计

- 按用户要求将 `data-forge-paper-material="opaque"` 恢复为持久默认值：输入框、项目条、环境信息、菜单/弹窗、已编辑文件卡、排队消息、目标与完成状态条重新使用完整明纸颜色和纸纹，不再模糊后方背景。
- V73 的单层透明磨砂实现未被删除，显式切换为 `frosted` 时仍可复现；默认路径和首帧契约均固定为 `opacity: 1` 与 `backdrop-filter: none`，避免启动或重挂载时短暂闪现透明框。
- 设置页继续自动使用项目对话同款场景遮罩，保留对整页过亮问题的修复；此遮罩与框体材质互相独立。

## 2026-08-16 — V73 透明磨砂明纸与设置页对话遮罩

- 保留 V72 不透明明纸为 `data-forge-paper-material="opaque"` 可回退材质，V73 默认使用 `frosted`：输入框、项目条、环境信息、菜单/弹窗、已编辑文件卡、排队消息、目标与完成状态条均由单一半透明载体模糊当前场景，低透明纹理仅负责纸纤维和雕边。
- 排队/目标栈取消每行重复的不透明纸层，由连续栈底承载磨砂，行内只保留分隔与细节；完成状态条改用与输入框一致的纸色、透明度和模糊参数。
- 设置页保持透明内容层，但根背景自动切换到当前场景的项目对话遮罩值；离开设置后由同一结构刷新自动解除，不改变战斗/风景选择。
- 浏览器回归同时发现并修正通用弹层曾被强制改为 `position: relative` 的几何风险；现在菜单、对话框、图片查看器和审阅卡全部保留原生定位与命中区。

## 2026-08-16 — 活动背景精简为 17 张

- 按用户截图确认并删除风景 S04 `sunlit-mountain-vista`、战斗 B03 `storm-bearer` 与 B12 `bear-crush` 的活动编号资源。
- 战斗与风景序列分别收敛为 11 张和 6 张；仅重排剩余条目的连续 `order`，稳定 `slot` 不变。
- 当前图库为 5,334,166 bytes / 35,063,992 解码像素，最坏双图过渡仍为 5,337,600 像素。
- 图片查看器首帧直接复用根节点已解码的当前背景，并以透明底透出同一场景；不再等待弹层标记写入后才从黑色后备底切换。
...
## 2026-08-13 — 小八戒 V3 捧腹大笑动作发布

- 按用户明确选择恢复 `little-bajie-v3-inart`，否决 V4 模型；V4 资产仅作为历史证据保留，不进入准备、打包或安装链。
- 用获准 V3 的五帧捧腹大笑替换 hover/jumping 与 active work/running；waving、failed、review 在官方固定帧数内采用成对关键姿势停顿，降低可见姿势变化频率且不改共享动画调度器。
- rows 0–2、6、9–10 与旧 V3 逐像素一致；新 atlas SHA-256 为 `e9f7114bd8873a66d77fc45ef1a3e0751e936436b8d0ea1a773c7792f521e3c0`，v2 严格验证通过，透明 RGB 残留与洋红边缘残留均为 0。
- 发布策略、准备脚本、最小包合同和用户文档同步到只发布 V3 小八戒；小悟空 V5 仍 pending，旧小悟空仍 frozen。
...
- 新增 `start.cmd` / `stop-theme.cmd`：profile、请求、事件和日志全部位于解压目录 `.wukong-runtime`；使用 Codex 包内 Node 24 与原生 WebSocket，不需要 npm 或全局 Node。
