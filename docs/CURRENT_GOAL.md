...
## V61 / 0.15.0 非宠物主题发布基线（2026-08-14）

- 新建任务页沿用官方“悟空”书法与朱印的深/浅双资产和原生 56×56 锚点；168×168 绘制尺寸、DOM、布局与命中区均不变化。
- 每个活动场景显式选择浅色/深色、平坦/复杂四档 `heroProfile`。V61 将四档字标统一提升为满不透明度，并分别校准亮度、饱和度、对比度与相反明暗的紧边缘分离：深墨字标加浅边，骨色字标加深边，解决暗蓝、烟尘、雪地和强光复杂背景下字标重新变灰或融入画面的现象；不使用统一发光、动画或全屏滤镜。
- 20 个背景的 fixture 全页矩阵与 computed style 合同共同作为发布前视觉门；本机安装后还须以真实官方 renderer 验证 `v61-visible-wordmark` 和对应字标 computed style。
- 本发布基线只覆盖非宠物主题。两只宠物及授权结论保持现状，不在 V61 中修改，也不作为本轮安装与主题视觉验收的组成部分。
