...
当前风景按雪山、夕阳、峡谷循环。图片资产不使用亮度、饱和度或对比度滤镜；新对话页不加遮罩，深色对话使用逐图配置，浅色对话使用白色薄遮罩。运行时没有定时轮换。用户仅使用 start.cmd 启动；开发者可通过 scripts/manage-backgrounds.ps1 管理资源，写入前会建立本地备份。新图片仍需通过字节、像素和双图过渡预算。
...
历史素材说明中的旧路径仅记录来源；旧图、宠物与制作资料已取消跟踪并保留本地，不进入当前运行包。
