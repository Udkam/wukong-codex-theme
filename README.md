...
当前源码版本为 **0.14.7 / V52.2**。主题提供 22 张有序背景、原生坐标上的四角纸面输入器、侧栏选中纸带、环境信息纸面以及清晰的运行进度状态色。
...
- `Ctrl+Alt+T` 原位隐藏或重新显示新建任务页的“此去，欲破何局？”题字；该选择在当前窗口内跨背景、任务和对话保持，重启后默认显示。
...
- 开关新建任务页题字：`Ctrl+Alt+T`（当前窗口内保持，重启后默认显示）
